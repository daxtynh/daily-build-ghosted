...
function getStatusColor(status: ApplicationStatus, days: number): string {
  if (status === "responded") return "bg-green-500/20 text-green-400 border-green-500/30";
  if (status === "rejected") return "bg-red-500/20 text-red-400 border-red-500/30";
  if (status === "ghosted" || days >= GHOST_THRESHOLD_DAYS) return "bg-purple-500/20 text-purple-400 border-purple-500/30";
...
export default function Home() {
  ...
  const stats = {
    ...
    ghosted: applications.filter(
      (a) => a.status === "ghosted" || (a.status === "applied" && a.daysSinceApplied >= GHOST_THRESHOLD_DAYS)
    ).length,
    responded: applications.filter((a) => a.status === "responded").length,
    rejected: applications.filter((a) => a.status === "rejected").length,
    waiting: applications.filter(
      (a) => a.status === "applied" && a.daysSinceApplied < GHOST_THRESHOLD_DAYS
    ).length,
    avgWaitDays:
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-40 right-20 opacity-10">
          <Ghost className="w-48 h-48 text-blue-500 animate-float" style={{ animationDelay: "1s" }} />
        </div>
        ...
        <div className="relative max-w-6xl mx-auto px-4 pt-16 pb-12">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-zinc-400 max-w-2xl mx-auto">
              Track which companies ghost you. See your stats. Know you&apos;re not alone.
            </p>
            <p className="text-sm text-zinc-500 mt-2">
              44% of job seekers get ghosted. The average wait is 21+ days.
            </p>
          </div>
          ...
          {applications.length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-zinc-900/50 border border-purple-500/30 rounded-xl p-4 card-hover">
                ...
                <div className="text-3xl font-bold text-purple-400">{stats.ghosted}</div>
                <div className="text-xs text-zinc-500">{stats.ghostRate}% ghost rate</div>
              </div>
              ...
              <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 card-hover">
                ...
                <div className="text-3xl font-bold text-white">{stats.avgWaitDays}d</div>
              </div>
            </div>
          )}
          ...
          {showForm && (
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 max-w-md w-full">
                <h2 className="text-xl font-semibold text-white mb-4">Add Application</h2>
          ...
          {applications.length > 0 ? (
            <div className="space-y-3">
              {applications.map((app) => {
                const effectiveStatus =
                  app.status === "applied" && app.daysSinceApplied >= GHOST_THRESHOLD_DAYS
                    ? "ghosted"
                    : app.status;
                return (
                  <div
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          >
                            {getStatusLabel(effectiveStatus, app.daysSinceApplied)}
                          </span>
                        </div>
                        ...
                        {app.notes && (
                          <p className="text-zinc-500 text-xs mt-2 italic">{app.notes}</p>
                        )}
                      </div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-zinc-600 text-sm">Add your first application to start tracking.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-zinc-900/30 border-t border-zinc-800 py-16">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-10">
            ...
            <p className="text-zinc-400">Based on industry data from 1,000+ job seekers</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 text-center card-hover">
              ...
              <p className="text-zinc-400 text-sm">of job seekers cite ghosting as their #1 frustration</p>
            </div>
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 text-center card-hover">
              <Clock className="w-8 h-8 text-blue-400 mx-auto mb-3" />
              <div className="text-4xl font-bold text-white mb-1">21 days</div>
              <p className="text-zinc-400 text-sm">average time before a response (if any)</p>
            </div>
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 text-center card-hover">
              <Users className="w-8 h-8 text-green-400 mx-auto mb-3" />
              <div className="text-4xl font-bold text-white mb-1">100-200</div>
              <p className="text-zinc-400 text-sm">applications sent by average job seeker</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-purple-900/30 to-blue-900/30 border-t border-zinc-800 py-16">
        <div className="max-w-xl mx-auto px-4 text-center">
          ...
          <p className="text-zinc-400 mb-6">
            We&apos;re building company ghosting reports and weekly reminders. Get early access.
          </p>
          {emailSubmitted ? (
      ...
      <footer className="border-t border-zinc-800 py-8">
        <div className="max-w-6xl mx-auto px-4 text-center text-zinc-500 text-sm">
          <p>Built with frustration and solidarity. Data stored locally in your browser.</p>
